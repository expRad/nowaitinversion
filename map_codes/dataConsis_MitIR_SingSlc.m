function [dataConsisMit,residualMit] = dataConsis_MitIR_SingSlc(raw,FT_op,incr,nArms,fit,cmap,wfull,ite)   
    ...
    residualMit = zeros(size(fit,1),size(fit,2),size(fit,3),size(fit,3));
    dataConsisMit = zeros(size(fit,1),size(fit,2),size(fit,3),size(fit,3));
...
%     disp(slice)
    ...
    for frames = 1:incr:nArms
        ...
        disp(strcat("DataConsis_MitIR ite=", num2str(ite)," ", num2str(frames), " / ", num2str(nArms)))
    ...
    residualMit(slice,:,:,:) = resid;
    dataConsisMit(slice,:,:,:) = result_dtCons;
